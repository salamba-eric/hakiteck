...
function renderProjects() {
  ...
  projects.forEach(project => {
    ...
    card.innerHTML = `
      ...
      <p class="border-t border-[#9cabba] border-dashed text-white text-sm font-medium leading-normal">${project.category}:</p>
      <div class="flex justfy-center items-center gap-1 flex-wrap">
        ${project.tools.map(tool => `
          <p class ="border border-[#9cabba] rounded-xl p-2 text-white text-xs font-normal leading-normal">${tool}</p>
        `).join("")}
      </div>
    `;
